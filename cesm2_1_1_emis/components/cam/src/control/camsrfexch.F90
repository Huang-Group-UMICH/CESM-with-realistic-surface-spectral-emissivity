...
!---------------------------------------------------------------------------
  ...
  type cam_out_t 
     ...
     real(r8)::flwds_spec(pcols,16)  !spectral downward flux at surface  !Added on June26, 2019 by Xiuhong Chen
     real(r8)::emis_spec(pcols,16)   ! spectral surface emissivity  !Added on June26, 2019 by Xiuhong Chen
...
!---------------------------------------------------------------------------
  ...
  type cam_in_t    
     ...
     real(r8) :: tlai(pcols)             ! leaf area index     !! added by Xiuhong Chen on June26, 2019
     real(r8) :: ts_atm(pcols)           ! surface radiative temperature from t_rad ! added by Xiuhong Chen on June26, 2019
     real(r8) :: srf_emis_spec(pcols,16) ! surface spectral emissivity ! added by Xiuhong Chen on June26, 2019
...
!
    ...
    do c = begchunk,endchunk
       ...
       cam_in(c)%tlai     (:) = posinf    ! added by Xiuhong Chen on June26, 2019
       cam_in(c)%ts_atm     (:) = posinf    ! added by Xiuhong Chen on June26,2019
       cam_in(c)%srf_emis_spec(:,:) = 1.0 ! added by Xiuhong Chen on June26,2019
...
!
    ...
    do c = begchunk,endchunk
       ...
       cam_out(c)%flwds_spec(:,:) = 0._r8  ! Added on June26, 2019 by Xiuhong Chen
       cam_out(c)%emis_spec(:,:) = 0._r8  ! Added on June26, 2019 by Xiuhong Chen
