...
contains
  ...
  subroutine lnd_import( bounds, x2l, glc_present, atm2lnd_inst, glc2lnd_inst)
    ...
    !
    ! !LOCAL VARIABLES:
    integer  :: g,i,k,nstep,ier,j      ! indices, number of steps, and error code !Xiuhong Chen added j on June26,2019
    ...
    do g = bounds%begg,bounds%endg
       ...
       ! change the sign to indicate addition of water to system.
             ...
             !!!!!!!!!!!!!! Xiuhong Chen added on June26, 2019
               do j=1,16
                  atm2lnd_inst%lwdn_spec(g,j)   =x2l(index_x2l_Faxa_lwdn_spec(j),i)
                  atm2lnd_inst%emis_spec(g,j)   =x2l(index_x2l_Faxa_emis_spec(j),i)
                enddo
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
  ...
  subroutine lnd_export( bounds, lnd2atm_inst, lnd2glc_inst, l2x)
    ...
    !
    ! !LOCAL VARIABLES:
    integer  :: g,i,k,j ! indices ! Xiuhong Chen added j on June26,2019
    ...
    do g = bounds%begg,bounds%endg
       ...
       l2x(index_l2x_Sl_tlai,i)        =  lnd2atm_inst%tlai(g)  ! added by Xiuhong Chen on June26, 2019
       l2x(index_l2x_Sl_ts_atm,i)        =  lnd2atm_inst%ts_atm(g)  !added by Xiuhong Chen on June26, 2019
       do j=1,16
          l2x(index_l2x_Sl_srf_emis_spec(j),i) = lnd2atm_inst%srf_emis_spec(g,j)! Xiuhong Chen added on June26,2019
       enddo
