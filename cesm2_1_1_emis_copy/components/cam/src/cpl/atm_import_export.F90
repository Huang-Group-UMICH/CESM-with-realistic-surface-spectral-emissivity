...
contains
  ...
  subroutine atm_import( x2a, cam_in, restart_init )
    ...
    use ppgrid    ,     only: begchunk, endchunk, pcols ! Xiuhong Chen added pcols on June26, 2019     
    ...
    use time_manager  , only: is_first_step,  get_curr_date  ! Xiuhong Chen added get curr_date here on June26, 2019
...
!   !!!!!!!!!!!!!!!!!!!!
    ...
    do c=begchunk,endchunk
	  ...
	  cam_in(c)%landfrac(i)  =  x2a(index_x2a_Sf_lfrac, ig)
          cam_in(c)%tlai(i)      =  x2a(index_x2a_Sl_tlai,  ig)    !added by Xiuhong Chen on June26, 2019
           cam_in(c)%ts_atm(i)        =sqrt(sqrt((cam_in(c)%lwup(i)/shr_const_stebol)))  ! added by Xiuhong Chen, June26, 2019
           do j=1,16
             cam_in(c)%srf_emis_spec(i,j) =1.0 ! added by Xiuhong Chen on June26, 2019
           enddo
  ...
  subroutine atm_export( cam_out, a2x )
    ...
    integer :: i,m,c,n,ig,j       ! indices! Xiuhong Chen added j on June26,2019
    integer :: ncols            ! Number of columns
    !-----------------------------------------------------------------------
    ...
    do c=begchunk, endchunk
	  ...
	  a2x(index_a2x_Sa_dens   ,ig) = cam_out(c)%rho(i)
         ...
         do j=1,16
           a2x(index_a2x_Faxa_lwdn_spec(j) ,ig) = cam_out(c)%flwds_spec(i,j)!added on June26, 2019 by Xiuhong Chen
           a2x(index_a2x_Faxa_emis_spec(j) ,ig) =cam_out(c)%emis_spec(i,j)!added on June26, 2019 by Xiuhong Chen
          enddo
