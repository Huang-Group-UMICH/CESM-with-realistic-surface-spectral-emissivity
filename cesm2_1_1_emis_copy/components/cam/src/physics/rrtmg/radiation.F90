...
use physconst,           only: cappa, cpair, stebol ! Xiuhong Chen added stebol on June 27, 2019

use time_manager,        only: get_nstep, is_first_restart_step, &
                               get_curr_calday, get_step_size, is_first_step !Xiuhong Chen added is_first_step here on June26, 2019
...
subroutine radiation_tend( &
   ...
   use parrrtm,         only: nbndlw   ! Xiuhong Chen added on June 27, 2019
   ! Arguments
   ...
   !--------------------------------------------------------------------------------------
    !    Xiuhong Chen added here on June26, 2019
   integer :: ilats(pcols),ilons(pcols),j
   real :: surface_emis(pcols,nbndlw)  ! surface band-by-band emissivity
   real(8) :: lwdn_spec(nbndlw, pcols)    ! spectral download LW flux  !
   real ::  desert_emis(nbndlw),water_emis(nbndlw),ice_emis(nbndlw),grass_emis(nbndlw), snow_emis(nbndlw) !
   real(8) :: Ts_LW(pcols)                ! surface temperature derived from longwave upward flux
...
!     data snow_emis / 0.9963, 0.9875, 0.9859, 0.9809, 0.9759,  0.9871,  0.9926,
!     0.9892, &
...
!!!!!!!!!!!!!!!!!!!  Xiuhong Chen changed here, June26, 2019
         ...
         do icall = N_DIAG, 0, -1
            ...
            if (active_calls(icall)) then
               ...
               call aer_rad_props_lw(icall, state, pbuf,  aer_lw_abs)
                ...
                !!!!!!!!!!!!!!!! Xiuhong Chen added on June26, 2019
                    do i=1,ncol
               ...
               call rad_rrtmg_lw( &
                  lchnk, ncol, num_rrtmg_levs, r_state, state%pmid,  &
                  aer_lw_abs, cldfprime, c_cld_lw_abs,surface_emis, qrl, rd%qrlc, & ! Xiuhong Chen added surface_emis on June 26, 2019
                  flns, flnt, rd%flnsc, rd%flntc, cam_out%flwds,     &
                  rd%flut, rd%flutc, fnl, fcnl, rd%fldsc,            &
                  lu, ld , lwdn_spec) ! Xiuhong Chen added lwdn_spec on June26,2019)
